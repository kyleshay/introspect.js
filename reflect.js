var Reflect = function (file) {
        ...
        function inject(file) {
            ...
            var privs = file.match(/((var )?\s*?(\w\.*?)=\s*?)?function(\s*?(\w\.*?))?\(/g), // get funcs
                replace = "return Public;", //this should be passed in as a param
                instance = eval("new (" + file.replace(replace, null
                    ...
                    + "var fn=funcs[i].replace(/function|=|\\s.*?|\\(|var /g,'');" //remove tokens for name
